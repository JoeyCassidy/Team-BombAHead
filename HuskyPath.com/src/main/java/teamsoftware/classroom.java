...
public class classroom {
	 ...
	 * @param title - the title of the new post (will be NULL for responses)
	 * @param description - the body of the new post
	 ...
	 * @param anonSwitch - boolean determines if they want to show up anonymous or not
	 */
	public void answerQuestion(String title, String description, HashMap<String,post> listOfPosts, student user, Date time, Boolean anonSwitch) {
//		might need to change due to potentially misunder standing concept
		post newReply = new post(title, description, new HashMap<String,post>(),user,time,anonSwitch);
		listOfPosts.put(user.Email + "_" + time.toString(), newReply);
	}
}
